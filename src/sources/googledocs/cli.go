...
// RunLogin handles the OAuth loopback (PKCE) flow for Google Docs.
...
<p>You can close this window and return to the terminal.</p>
	...
	case code := <-codeChan:
		...
		cfg.Sources["googledocs"] = core.SourceConfig{Enabled: true}
...
// RunReset removes Google Docs data after prompting for confirmation.
...
// InfoLines returns indented lines for the `info` command Google Docs section.
func InfoLines(dataDir string) []string {
	...
	} else if email, err := os.ReadFile(filepath.Join(dataDir, "googledocs_email.txt")); err == nil && len(email) > 0 {
	...
	defer cancel()
	var n int
	if err := src.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM documents").Scan(&n); err != nil {
