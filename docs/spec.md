...
3. **Recency boost** — multiply the weighted BM25 score by `1 + 2*exp(-t/3.5) + 5*exp(-t/800)` where `t` is the entry's age in days. This bi-exponential gives today → ~8x, one week → ~6x, one year → ~4x, two years → ~3x, three years → ~2x, ten years → ~1x. The short-term component (τ=3.5 days) captures the initial freshness drop; the long-term component (τ=800 days) provides a sustained boost for content within the last few years. Entries without a stored timestamp receive a 1.0 multiplier (no boost). The boost is applied after BM25 candidate retrieval so very recent results naturally surface above older ones with equivalent textual relevance.
