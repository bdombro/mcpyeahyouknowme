...
| `calendar_event` | Google Calendar | Event summary/location/organizer rows with human-readable date appended (e.g. `2024-06-15 June 2024 Saturday`) |
...
3. **Temporal multiplier** — applied after hierarchy weighting; the function used depends on content type:
   - **Most content** (`recencyMultiplier`): `1 + 3*exp(-t/2.5) + 0.5*exp(-t/90) + 1.5*exp(-t/700)` where `t` is age in days. Today → ~6x, one week → ~3x, one month → ~2.8x, one year → ~1.9x, ten years → ~1x.
   - **Calendar events** (`calendar_*` types, `futureProximityMultiplier`): `1 + futureBias + 4*exp(-|t|/3) + 1*exp(-|t|/30)` where `t` is days from now (positive = future) and `futureBias` is +1.5 for future dates. Approximate values: tomorrow ~6.8x, yesterday ~5.3x, next week ~3.5x, last week ~2.3x. This ensures upcoming events rank above past ones.
   - **Tasks** (`task` type): same `futureProximityMultiplier` as calendar, then multiplied by a **status multiplier**: `needsAction` (incomplete) → 1.5x, `completed` → 0.6x, other → 1.0x. Incomplete tasks with upcoming due dates therefore rank highest; completed tasks with past due dates rank lowest.
   - Entries without a stored timestamp receive a 1.0 multiplier (no boost). G Suite entries (calendar, Gmail, Docs, Sheets, Slides, Tasks) set `Timestamp` during indexing so they participate in temporal ranking.
4. **Date range filters** — the `search` tool accepts optional `after` and `before` RFC3339 parameters. When provided, only entries whose stored `timestamp` falls within the given range are returned. This applies as a SQL filter before BM25 scoring.
5. **Candidate pool and limit** — FTS5 returns up to `5 × limit` BM25 candidates (default limit 20 → 100 candidates). Each candidate is hydrated, scored as `|BM25| × hierarchy × temporal`, sorted by final score descending, then truncated to `limit`. Truncation happens after temporal scoring so a weaker BM25 match can still appear in the top results when it is newer or more actionable.
