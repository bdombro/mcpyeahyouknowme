...
3. **Recency boost** — multiply the weighted BM25 score by `1 + 5 / (1 + ageDays)` where `ageDays` is the fractional age of the entry's `timestamp`. This gives today's entries a ~6x multiplier, entries from one week ago ~1.6x, and entries from one year ago ~1.01x. Entries without a stored timestamp receive a 1.0 multiplier (no boost). The boost is applied after BM25 candidate retrieval so very recent results naturally surface above older ones with equivalent textual relevance.
